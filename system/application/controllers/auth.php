...
class Auth extends Controller
{
	...
	function adduser(){
		...
		if ($this->dx_auth->is_logged_in() AND $this->session->userdata('DX_role_id') != 3 ){
			...
			$loginRoleID = $this->session->userdata('DX_role_id');
			$loginUserID = $this->session->userdata('DX_user_id');
			$loginCenterID = $this->session->userdata('DX_center_id');
			
		
			if( $this->input->post("type") == 3  ){
				//$data["supervisor_id"] = ($loginRoleID  == 4)? $loginUserID:$this->input->post("supervisor");
			}
			...
			$data["centerid"] = ($loginRoleID  == 4)?$loginCenterID:$this->input->post("centers");
	...
	function updateUser(){
		if ($this->dx_auth->is_logged_in() AND $this->session->userdata('DX_role_id') != 3 ){
			...
			$data["role_id"] 	= $this->input->post("utype");
			$data["centerid"] 	= $this->input->post("centers");
			$data["fname"] 		= $this->input->post("fname");
			$data["lname"] 		= $this->input->post("lname");
			$data["username"] 	= $this->input->post("username");
			$data["password"] 	= $this->input->post("password");
