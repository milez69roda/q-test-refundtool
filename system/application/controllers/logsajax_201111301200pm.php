...
class LogsAjax extends Controller {
	...
	function getLogListing1(){
		...
		$aColumns = array(  'fname','lname','log_status_name', 'audited_name',
		...
		if( $this->roleId == 3 ){
			$sWhere .= " updated_date <> '' AND users.centerid = ".$this->centerId;
			
		}else{
			$sWhere .= " updated_date <> '' AND users.centerid <> '' ";
		}
	...
	function download(){
		...
		$sql = 'SELECT 
				...
				WHERE DATE_FORMAT(updated_date,"%Y-%m-%d") BETWEEN "'.$date_from.'" AND "'.$date_to.'"
					'.$status.'
